...
class StartRecordingDialogContent extends AbstractStartRecordingDialogContent<IProps> {
    ...
    _renderNoIntegrationsContent() {
        ...
        return (
            ...
            >
                ...
                <Text className="recording-title">{label}</Text>
                {switchContent}
            </Container>
        );
    }
    ...
    _renderUploadToTheCloudInfo() {
        const { _hideStorageWarning, isVpaas, selectedRecordingService, t } =
            this.props;
        ...
        return (
            <Container className="recording-info" key="cloudUploadInfo">
                <Image className="recording-info-icon" src={ICON_INFO} />
            ...
            <Container>
                <Container
                    className={`recording-header ${
                        this._shouldRenderNoIntegrationsContent()
                            ? "recording-header-line"
                            : ""
                    }`}
                >
                    <Container className="recording-icon-container">
                        <Image className="recording-icon" src={DROPBOX_LOGO} />
                    </Container>
                    <Text className="recording-title">
                ...
                {selectedRecordingService === RECORDING_TYPES.LOCAL && (
                    <>
                        ...
                        <Text className="local-recording-warning text">
                            Убедитесь, что вы выбрали правильную вкладку для
                            записи видео и аудио. Запись в настоящее время
                            ограничена 1 ГБ, что составляет около 100 минут.
                        </Text>
                        {_localRecordingNoNotification &&
                            !localRecordingOnlySelf && (
                                <Text className="local-recording-warning notification">
                                    {t(
                                        "recording.localRecordingNoNotificationWarning"
                                    )}
                                </Text>
                            )}
                    </>
