...
local function get_occupant_details(occupant)
    ...
    local occupant_name;
    if presence then
        occupant_name = presence:get_child("nick", NS_NICK) and presence:get_child("nick", NS_NICK):get_text() or 'Неизвестный пользователь';
    else
        occupant_name = 'Неизвестный пользователь'
    end
